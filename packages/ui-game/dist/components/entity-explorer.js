...
export function EntityImage({ entity }) {
    ...
    return (_jsx("img", { src: entity.image, alt: entity.name, onError: () => setError(true), className: `w-full h-full p-2 transition-transform duration-500 group-hover:scale-110 drop-shadow-[0_5px_15px_rgba(0,0,0,0.5)] ${entity.kind === "characters" ? "object-contain object-bottom p-0" : "object-contain"}` }));
}
export function EntityExplorer({ kind, compact = false, kindLabels = defaultKindLabels, apiEndpoint = "/api/entities", getDetailHref, renderElementBadge, }) {
    ...
    return (_jsxs("section", { ref: explorerRef, className: "entity-explorer", "aria-busy": loading, children: [_jsxs("form", { className: "entity-search", onSubmit: submit, children: [_jsxs("label", { children: [_jsxs("span", { className: "sr-only", children: ["Search ", kind ? kindLabels[kind] ?? kind : "all entities"] }), _jsx("input", { onChange: (event) => setQuery(event.target.value), placeholder: kind ? `Search ${kind}…` : "Search characters, weapons, domains…", type: "search", value: query })] }), _jsx("button", { type: "submit", children: "Search graph" })] }), _jsxs("div", { className: "entity-result-meta", children: [_jsx("span", { children: loading
                    ...
                    return (_jsxs(Link, { href: detailHref, className: "group flex flex-col rounded-xl overflow-hidden bg-[var(--surface-sunken)] border border-white/5 shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(0,0,0,0.5)] cursor-pointer h-64 sm:h-72", style: { borderBottom: `4px solid ${rarityColor}` }, children: [_jsxs("div", { className: "relative flex-1 bg-[var(--surface)] overflow-hidden flex items-center justify-center p-2", children: [_jsx("div", { className: "absolute inset-0 opacity-20 group-hover:opacity-40 transition-opacity duration-500", style: { background: `radial-gradient(circle at center, ${rarityColor} 0%, transparent 70%)` } }), _jsx("div", { className: "relative w-full h-full transition-transform duration-500 ease-out group-hover:scale-110 flex items-center justify-center z-10", children: _jsx(EntityImage, { entity: entity }) }), ElementIcon && (_jsx("div", { className: "absolute top-2 right-2 z-20 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-black/50 flex items-center justify-center backdrop-blur-md border border-white/10 shadow-sm", title: entity.element, children: ElementIcon })), _jsx("div", { className: "absolute top-2 left-2 z-20 px-2 py-0.5 rounded bg-black/60 backdrop-blur-md border border-white/5 text-xs text-white/80 font-bold uppercase tracking-widest shadow-sm", children: kindLabels[entity.kind] ?? entity.kind })] }), _jsxs("div", { className: "relative z-20 bg-gradient-to-b from-[var(--surface-sunken)] to-[var(--surface-raised)] border-t border-white/5 p-3 flex flex-col justify-center h-[76px] sm:h-[84px] shrink-0", children: [entity.rarity && (_jsx("div", { className: "flex gap-0.5 mb-1 opacity-90 justify-center", children: Array.from({ length: Math.min(entity.rarity, 5) }).map((_, i) => (_jsx(Star, { size: 10, fill: "#ffc83d", color: "#ffc83d", className: "drop-shadow-sm" }, i))) })), _jsx("h2", { className: "text-white font-bold text-center text-xs sm:text-sm leading-snug line-clamp-2 drop-shadow-sm group-hover:text-[var(--accent)] transition-colors", title: entity.name, children: entity.name })] })] }, `${entity.kind}:${entity.id}`));
